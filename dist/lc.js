...
var blocks=doc.queryAllBlocks();for(var i=0;i<blocks.length;i++){if(doc.queryBlockEntities(blocks[i]).length==0&&blocks[i]!=doc.getModelSpaceBlockId()){op.deleteObject(doc.queryBlock(blocks[i]));}}
...
var block=new RBlock(doc,'BB',new RVector(0,0));op.addObject(block,false);di.applyOperation(op);doc.setCurrentLayer(doc.getLayer0Id());var op2=new RAddObjectsOperation(false);for(var i=0;i<nodes.length;i++){var node=nodes[i];var a=new RVector(node.pos[0],node.pos[1]),b=new RVector(node.pos[0]+node.w,node.pos[1]+node.h);var box=new RBox(a,b);var _box=new RPolylineEntity(doc,new RPolylineData());_box.setShape(box.getPolyline2d());_box.setBlockId(block.getId());op2.addObject(_box,false);}
var br=new RBlockReferenceEntity(doc,new RBlockReferenceData(block.getId(),new RVector(0,0),new RVector(1,1),0));br.setBlockId(doc.getModelSpaceBlockId());op2.addObject(br,false);di.applyOperation(op2);qDebug((Date.now()-before)/1e3,'s');})();(function(){var before=Date.now();var doc=getDocument();var di=getDocumentInterface();var blocks=doc.queryAllBlockReferences();var model=doc.getModelSpaceBlockId();var op=new RModifyObjectsOperation(false);for(var i=0;i<blocks.length;i++){var b=doc.queryEntity(blocks[i]);var pos=b.getPosition();var rot=b.getRotation();if(b.getReferencedBlockName()=='BB'){continue;}
