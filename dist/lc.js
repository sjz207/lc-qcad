...
var entities=doc.queryAllEntities();var op=new RAddObjectsOperation(false);for(var i=0;i<entities.length;i++){var ent=doc.queryEntity(entities[i]);if(isBlockReferenceEntity(ent)){var itms=doc.queryBlockEntities(ent.getReferencedBlockId()),itms2=itms.map(function(itm){return doc.queryEntity(itm);}).filter(function(itm){return isPolylineEntity(itm)&&itm.isClosed()&&itm.getLayerName()!=cfg['engraving-layer-name'];});var filtered=[];var op2=new RModifyObjectsOperation(false);for(var j=0;j<itms2.length;j++){var itmA=itms2[j],shA=itmA.castToShape();var c=0;for(var k=0;k<itms2.length;k++){if(j!=k){var itmB=itms2[k],shB=itmB.castToShape();if(shB.containsShape(shA)){c++;break;}}}
