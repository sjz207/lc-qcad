...
di.applyOperation(op);})();(function(){var doc=getDocument();var di=getDocumentInterface();var entities=doc.queryAllEntities(false,true);var filtered=[],pts=[];for(var i=0;i<entities.length;i++){var obj=entities[i],ent=doc.queryEntity(obj);if(isArcEntity(ent)||isLineEntity(ent)||(isPolylineEntity(ent)&&!ent.isClosed())){var sPt=ent.getStartPoint(),ePt=ent.getEndPoint();pts.push({'x':sPt.x,'y':sPt.y,'obj':obj,'end':0});pts.push({'x':ePt.x,'y':ePt.y,'obj':obj,'end':1});filtered.push(obj);}}
...
shs.push({'shape':sh2,'id':near[0].obj});return true;}}}else{var sh=shs[0];var pt=sh.shape.getStartPoint();var nearest=tree.nearest({'x':pt.x,'y':pt.y},5);for(var i=0;i<nearest.length;i++){var near=nearest[i];var ent=doc.queryEntity(near[0].obj);if(near[1]<1e-5&&near[0].obj!=sh.id&&near[0].obj!=shs[shs.length-1].id&&layId==ent.getLayerId()){var sh2=ent.castToShape().clone();if(near[0].end==0){sh2.reverse();}
shs.unshift({'shape':sh2,'id':near[0].obj});return true;}}}
return false;}
var visited=[];for(var i=0;i<filtered.length;i++){var id=filtered[i];if(visited.indexOf(id)<0){var f=doc.queryEntity(id);var shapesA=[{'id':id,'shape':f.castToShape().clone()}];while(Search(shapesA,'right',f.getLayerId())){};while(Search(shapesA,'left',f.getLayerId())){};var ids=shapesA.map(function(s){return s.id;});Array.prototype.push.apply(visited,ids);var shapesB=[];for(var j=0;j<shapesA.length;j++){var s=shapesA[j].shape;if(isPolylineShape(s)){Array.prototype.push.apply(shapesB,s.getExploded());}else{shapesB.push(s);}}
var newPl=new RPolyline(shapesB);var pl=shapeToEntity(doc,newPl);pl.copyAttributesFrom(f.data());var op=new RAddObjectsOperation(false);op.addObject(pl,false);di.applyOperation(op);}}
