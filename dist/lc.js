...
else{break;}}}};exports.kdTree=kdTree;exports.BinaryHeap=BinaryHeap;}));var cfg={"paper-size":[500,750],"paper-padding":10,"engraving-layer-name":"Gravur","cutting-layer-name":"Schnitt","cutting-width":0.15,"packing-padding":3,"equal-sized-objects-dist":2,"gap-width":0.5,"gap-min-dist":25};(function(){var before=Date.now();var doc=getDocument();var di=getDocumentInterface();var layA=doc.queryLayer('New');if(isNull(layA)){layA=addLayer('New','Cyan');}
var layB=doc.queryLayer(cfg['engraving-layer-name']);var op=new RDeleteObjectsOperation(false);var hatches=doc.queryAllEntities(false,true,RS.EntityHatch);for(var i=0;i<hatches.length;i++){op.deleteObject(doc.queryEntity(hatches[i]));}
di.applyOperation(op);var op=new RModifyObjectsOperation(false);if(!isNull(layB)){layB.setLineweight(RLineweight.Weight000);layB.setColor(new RColor('Red'));op.addObject(layB,false);}
...
var blocks=doc.queryAllBlockReferences();for(var i=0;i<blocks.length;i++){var ent=doc.queryEntity(blocks[i]);var pos=ent.getPosition();var rot=ent.getRotation();var itms=doc.queryBlockEntities(ent.getReferencedBlockId());for(var j=0;j<itms.length;j++){var itm=doc.queryEntity(itms[j]);itm.rotate(rot);itm.move(pos);SetStyle(itm);op.addObject(itm,false);}
ent.setPosition(new RVector(0,0));ent.setRotation(0);ent.setLayerId(doc.getLayer0Id());op.addObject(ent,false);}
var all=doc.queryAllEntities(false,false,[RS.EntityArc,RS.EntityLine,RS.EntityCircle,RS.EntityPolyline]);for(var i=0;i<all.length;i++){var ent=doc.queryEntity(all[i]);SetStyle(ent);op.addObject(ent,false);}
di.applyOperation(op);var op=new RAddObjectsOperation(false);var other=doc.queryAllEntities(false,true,[RS.EntityCircle,RS.EntityPolyline]);for(var i=0;i<other.length;i++){var ent=doc.queryEntity(other[i]),sh=ent.castToShape();if(isCircleEntity(ent)){var rad=sh.getRadius(),c=sh.getCenter();var arcA=new RArc(c,rad,0,Math.PI),arcB=new RArc(c,rad,Math.PI,2*Math.PI);var entA=shapeToEntity(doc,arcA),entB=shapeToEntity(doc,arcB);entA.copyAttributesFrom(ent.data());entB.copyAttributesFrom(ent.data());op.addObject(entA,false);op.addObject(entB,false);op.deleteObject(ent);}else{var expl=sh.getExploded();for(var j=0;j<expl.length;j++){var newEnt=shapeToEntity(doc,expl[j].clone());newEnt.copyAttributesFrom(ent.data());op.addObject(newEnt,false);}
op.deleteObject(ent);}}
di.applyOperation(op);var op=new RDeleteObjectsOperation(false);var lines=doc.queryAllEntities(false,true,RS.EntityLine);for(var i=0;i<lines.length;i++){var ent=doc.queryEntity(lines[i]),sh=ent.castToShape();if(sh.getLength()<.1){op.deleteObject(ent);}}
...
di.applyOperation(op);qDebug((Date.now()-before)/1e3,'s');})();(function(){var before=Date.now();var doc=getDocument();var di=getDocumentInterface();var entities=doc.queryAllEntities(false,true);var filtered=[],pts=[];for(var i=0;i<entities.length;i++){var obj=entities[i],ent=doc.queryEntityDirect(obj);if(isArcEntity(ent)||isLineEntity(ent)){var sPt=ent.getStartPoint(),ePt=ent.getEndPoint();var layId=ent.getLayerId();pts.push({'x':sPt.x,'y':sPt.y,'obj':obj,'end':0,'endPt':ePt,'layId':layId});pts.push({'x':ePt.x,'y':ePt.y,'obj':obj,'end':1,'endPt':sPt,'layId':layId});filtered.push(obj);}}
function df(a,b){return(a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-b.y);}
var tree=new kdTree(pts,df,['x','y']);for(var i=0;i<pts.length;i++){pts[i].i=i;}
var dupl=[];for(var i=0;i<pts.length;i++){if(dupl.indexOf(pts[i].obj)<0){var nearest=tree.nearest(pts[i],5);for(var j=0;j<nearest.length;j++){if(nearest[j][1]<1e-5&&nearest[j][0].obj!=pts[i].obj&&dupl.indexOf(nearest[j][0].obj)<0){if(df(nearest[j][0].endPt,pts[i].endPt)<1e-5){var shA=doc.queryEntityDirect(nearest[j][0].obj).castToShape(),shB=doc.queryEntityDirect(pts[i].obj).castToShape();if(isArcShape(shA)&&isArcShape(shB)){if(shA.equals(shB,1e-5)){dupl.push(nearest[j][0].obj);}}else{dupl.push(nearest[j][0].obj);}}}}}}
function GetAng(a,b){var ang=Math.atan2(a.x*b.y-b.x*a.y,a.x*b.x+a.y*b.y);if(ang<0){ang+=2*Math.PI;}
return ang;}
function Mod(a,b){return((a%b)+b)%b;}
var skip=[];for(var i=0;i<pts.length;i++){if(skip.indexOf(i)<0&&dupl.indexOf(pts[i].obj)<0){var nearest=tree.nearest(pts[i],5);var objs=nearest.filter(function(p){return dupl.indexOf(p[0].obj)<0&&p[0].obj!=pts[i].obj&&p[1]<1e-5&&pts[i].layId==p[0].layId;});if(objs.length>1){throw new Error('Ambiguous connection found at coordinate ['+pts[i].x+', '+pts[i].y+'].');}else if(objs.length==1){var obj=doc.queryEntityDirect(objs[0][0].obj),sh=obj.castToShape();if(isLineEntity(obj)){if(objs[0][0].end==0){obj.setStartPoint(new RVector(pts[i].x,pts[i].y));}else{obj.setEndPoint(new RVector(pts[i].x,pts[i].y));}}else{var phi=Mod(obj.getEndAngle()-obj.getStartAngle(),2*Math.PI);if(obj.isReversed()){phi=2*Math.PI-phi;}
phi/=2;if(objs[0][0].end==1){var pt=obj.getStartPoint();}else{var pt=obj.getEndPoint();}
var v=new RVector(pt.x-pts[i].x,pt.y-pts[i].y),l=v.getMagnitude()/2;v.normalize();var d=l/Math.tan(phi);var f=(objs[0][0].end==0^obj.isReversed())?1:-1;var c=new RVector(pts[i].x+l*v.x-f*d*v.y,pts[i].y+l*v.y+f*d*v.x);var vA=new RVector(pts[i].x-c.x,pts[i].y-c.y),vB=new RVector(pt.x-c.x,pt.y-c.y);var r=vA.getMagnitude();vA.normalize();vB.normalize();var x=new RVector(1,0);var angA=GetAng(x,vA),angB=GetAng(x,vB);sh.setCenter(c);sh.setRadius(r);if(objs[0][0].end==0){sh.setStartAngle(angA);sh.setEndAngle(angB);}else{sh.setEndAngle(angA);sh.setStartAngle(angB);}}
var op=new RModifyObjectOperation(obj,false);di.applyOperation(op);skip.push(objs[0][0].i);}}}
function Search(shs,side,layId){if(side=='right'){var sh=shs[shs.length-1];var pt=sh.shape.getEndPoint();var nearest=tree.nearest({'x':pt.x,'y':pt.y},2);for(var i=0;i<nearest.length;i++){var near=nearest[i];if(near[1]<1e-5&&near[0].obj!=sh.id&&near[0].obj!=shs[0].id&&dupl.indexOf(near[0].obj)<0&&layId==near[0].layId){var ent=doc.queryEntityDirect(near[0].obj),sh2=ent.castToShape().clone();if(near[0].end==1){sh2.reverse();}
shs.push({'shape':sh2,'id':near[0].obj});return true;}}}else{var sh=shs[0];var pt=sh.shape.getStartPoint();var nearest=tree.nearest({'x':pt.x,'y':pt.y},2);for(var i=0;i<nearest.length;i++){var near=nearest[i];if(near[1]<1e-5&&near[0].obj!=sh.id&&near[0].obj!=shs[shs.length-1].id&&dupl.indexOf(near[0].obj)<0&&layId==near[0].layId){var ent=doc.queryEntityDirect(near[0].obj),sh2=ent.castToShape().clone();if(near[0].end==0){sh2.reverse();}
shs.unshift({'shape':sh2,'id':near[0].obj});return true;}}}
return false;}
var visited=[];var op=new RAddObjectsOperation(false);for(var i=0;i<filtered.length;i++){var id=filtered[i];if(visited.indexOf(id)<0&&dupl.indexOf(id)<0){var f=doc.queryEntityDirect(id);var shapes=[{'id':id,'shape':f.castToShape().clone()}];while(Search(shapes,'right',f.getLayerId())){};while(Search(shapes,'left',f.getLayerId())){};var ids=shapes.map(function(s){return s.id;});Array.prototype.push.apply(visited,ids);var newPl=new RPolyline(shapes.map(function(s){return s.shape;}));var pl=shapeToEntity(doc,newPl);pl.copyAttributesFrom(f.data());op.addObject(pl,false);}}
Array.prototype.push.apply(visited,dupl);for(var i=0;i<visited.length;i++){op.deleteObject(doc.queryEntityDirect(visited[i]));}
di.applyOperation(op);qDebug((Date.now()-before)/1e3,'s');})();(function(){var doc=getDocument();var di=getDocumentInterface();var entities=doc.queryAllEntities();var nodes=[];function BB(minX,maxX,minY,maxY){this.minX=minX;this.maxX=maxX;this.minY=minY;this.maxY=maxY;this.merge=function(other){return new BB(Math.min(this.minX,other.minX),Math.max(this.maxX,other.maxX),Math.min(this.minY,other.minY),Math.max(this.maxY,other.maxY));}
...
var block=new RBlock(doc,'BB',new RVector(0,0));op.addObject(block,false);di.applyOperation(op);var op2=new RAddObjectsOperation(false);for(var i=0;i<nodes.length;i++){var node=nodes[i];var a=new RVector(node.pos[0],node.pos[1]),b=new RVector(node.pos[0]+node.w,node.pos[1]+node.h);var box=new RBox(a,b);var box_=new RPolylineEntity(doc,new RPolylineData());box_.setShape(box.getPolyline2d());box_.setBlockId(block.getId());op2.addObject(box_,false);}
var br=new RBlockReferenceEntity(doc,new RBlockReferenceData(block.getId(),new RVector(0,0),new RVector(1,1),0));op2.addObject(br,false);di.applyOperation(op2);qDebug((Date.now()-before)/1e3,'s');})();(function(){var before=Date.now();var doc=getDocument();var di=getDocumentInterface();var blocks=doc.queryAllBlockReferences();var model=doc.getModelSpaceBlockId();var op=new RModifyObjectsOperation(false);for(var i=0;i<blocks.length;i++){var b=doc.queryEntity(blocks[i]);var pos=b.getPosition();var rot=b.getRotation();if(b.getReferencedBlockName()=='BB'){continue;}
...
di.applyOperation(op);qDebug((Date.now()-before)/1e3,'s');})();function GetCX(pts){var n=pts.length;var s=0;for(var i=1;i<n;i++){if(pts[i].y<pts[s].y){s=i;}}
...
cxEnt.setLayerId(layA.getId());obbEnt.setLayerId(layB.getId());op.addObject(cxEnt,false);op.addObject(obbEnt,false);}else{var expl=ent.getExploded();for(var j=0;j<expl.length;j++){var newEnt=shapeToEntity(doc,expl[j].clone());newEnt.setLayerId(layName==cfg['engraving-layer-name']?layD.getId():layC.getId());op.addObject(newEnt,false);}}
op.deleteObject(ent);}}
di.applyOperation(op);qDebug((Date.now()-before)/1e3,'s');})();